...
export const TaskTimeLogsGantt: React.FC<{ taskId: string }>=({ taskId })=>{
  const { user } = useAuth();
  ...
  const start = async () => {
    ...
    setLoading(true);
    const { error } = await supabase.from('gantt_time_logs').insert({ task_id: taskId, user_id: currentUserId, note: note || null });
    setLoading(false);

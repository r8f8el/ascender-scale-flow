...
const Navbar = () => {
  ...
  const getLinkClasses = (path: string) => {
    return `text-gray-700 px-3 py-2 text-sm font-medium transition-colors rounded-md relative ${location.pathname === path ? 'text-gray-900' : ''}`;
  };

  const getMobileLinkClasses = (path: string) => {
    return `text-gray-700 block px-3 py-2 text-base font-medium transition-colors rounded-md ${location.pathname === path ? 'text-gray-900' : ''}`;
  };

...
export const useTicketForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Buscar status padrão "Novo" ou "Aberto"
      ...
      // Se ainda não existir, criar um status padrão
      if (!statusData.data) {
        console.log('Creating default status...');
        const createStatusRes = await supabase
          .from('ticket_statuses')
          .insert({ name: 'Aberto', color: '#3B82F6' })
          .select('id')
          .single();
        
        if (createStatusRes.error) {
          throw createStatusRes.error;
        }
        statusData = createStatusRes;
      }

      const ticketData = {
        ...formData,
        status_id: statusData.data.id,

...
const AdminFPADashboard = () => {
  const [selectedClient, setSelectedClient] = useState<string>('');
  const [selectedPeriod, setSelectedPeriod] = useState('');
  const [activeTab, setActiveTab] = useState('overview');

  const { data: clients = [], isLoading: clientsLoading } = useFPAClients();
  const { data: financialData = [], isLoading: dataLoading } = useFPAFinancialData(selectedClient);
  const { data: reports = [], isLoading: reportsLoading } = useFPAReports(selectedClient);
  const { data: varianceAnalysis = [], isLoading: varianceLoading } = useFPAVarianceAnalysis(selectedClient);
  const { data: periods = [], isLoading: periodsLoading } = useFPAPeriods(selectedClient);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Dashboard FP&A - Administração</h1>
          <p className="text-gray-600 mt-1">
            Gerencie e configure a plataforma de Planejamento e Análise Financeira
          </p>
        </div>
        ...
        <Card>
          <CardContent className="text-center py-12">
            ...
            <p className="text-gray-600">
              Escolha um cliente FP&A para gerenciar seus dados e análises
            </p>
          </CardContent>
        </Card>
      ) : (
        <>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
            ...
            <TabsContent value="overview" className="space-y-6">
              ...
              {/* Recent Financial Data with Admin Controls */}
              <Card>
                <CardHeader>
            ...
            <TabsContent value="drivers">
              <FPADriversManager clientId={selectedClient} />
            </TabsContent>

            <TabsContent value="scenarios">
              <FPAScenarioBuilder clientId={selectedClient} />
            </TabsContent>

            <TabsContent value="forecast">
              <FPARollingForecast clientId={selectedClient} />
            </TabsContent>
...
export default AdminFPADashboard;

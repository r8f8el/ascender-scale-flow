...
USING (
  EXISTS (
    ...
    WHERE kt.id = kanban_task_comments.task_id
      AND (kb.client_id = auth.uid() OR (EXISTS (SELECT 1 FROM admin_profiles WHERE id = auth.uid())))
  )
);
...
ON public.kanban_task_comments
FOR UPDATE USING (author_id = auth.uid() OR (EXISTS (SELECT 1 FROM admin_profiles WHERE id = auth.uid())))
WITH CHECK (author_id = auth.uid() OR (EXISTS (SELECT 1 FROM admin_profiles WHERE id = auth.uid())));

CREATE POLICY "Users can delete own kanban comments"
ON public.kanban_task_comments
FOR DELETE
USING (author_id = auth.uid() OR (EXISTS (SELECT 1 FROM admin_profiles WHERE id = auth.uid())));
...
USING (
  EXISTS (
    ...
    WHERE gt.id = gantt_task_comments.task_id
      AND (gp.client_id = auth.uid() OR (EXISTS (SELECT 1 FROM admin_profiles WHERE id = auth.uid())))
  )
);
...
ON public.kanban_time_logs
FOR UPDATE USING (user_id = auth.uid() OR (EXISTS (SELECT 1 FROM admin_profiles WHERE id = auth.uid())))
WITH CHECK (user_id = auth.uid() OR (EXISTS (SELECT 1 FROM admin_profiles WHERE id = auth.uid())));

CREATE POLICY "Users can delete own kanban time logs"
ON public.kanban_time_logs
FOR DELETE
USING (user_id = auth.uid() OR (EXISTS (SELECT 1 FROM admin_profiles WHERE id = auth.uid())));

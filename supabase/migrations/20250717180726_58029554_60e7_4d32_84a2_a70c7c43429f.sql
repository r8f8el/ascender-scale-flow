...
CREATE POLICY "Users can view messages from their rooms" ON public.chat_messages
  FOR SELECT USING (
    EXISTS (
      ...
      AND (chat_rooms.client_id = auth.uid() OR (EXISTS (SELECT 1 FROM admin_profiles WHERE id = auth.uid())))
    )
  );
...
CREATE POLICY "Users can upload their own files" ON storage.objects
  FOR INSERT WITH CHECK (
    bucket_id = 'files' AND 
    (auth.uid()::text = (storage.foldername(name))[1] OR (EXISTS (SELECT 1 FROM admin_profiles WHERE id = auth.uid())))
  );

-- Purge fictitious client/test data and all related records safely
BEGIN;

-- 1) Identify target client profiles to purge (test/dummy patterns)
...
-- 2) Identify related FPA clients for those profiles PLUS any FPA client with test-like company names
...
-- 3) Other dependent entities to clean up
...
-- 4) Delete children first (respecting possible FKs even if not explicitly listed)
-- Chat
...
-- Finally, the client profiles themselves

...
-- Corrigir a função invite_team_member_secure para usar uma alternativa ao gen_random_bytes
...
BEGIN
  ...
  invite_url := current_setting('app.base_url', true) || '/convite-equipe/cadastro?token=' || secure_token;
  IF invite_url IS NULL OR invite_url = '/convite-equipe/cadastro?token=' || secure_token THEN
    invite_url := 'https://klcfzhpttcsjuynumzgi.supabase.co/convite-equipe/cadastro?token=' || secure_token;
  END IF;

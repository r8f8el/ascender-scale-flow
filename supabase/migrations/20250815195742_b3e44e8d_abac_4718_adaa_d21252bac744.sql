...
CREATE POLICY "Usuários podem ver suas próprias solicitações" ON public.solicitacoes
  FOR SELECT USING (solicitante_id = auth.uid() OR aprovador_atual_id = auth.uid() OR (EXISTS (SELECT 1 FROM admin_profiles WHERE id = auth.uid())));
...
CREATE POLICY "Solicitantes podem atualizar suas solicitações" ON public.solicitacoes
  FOR UPDATE USING (solicitante_id = auth.uid() OR aprovador_atual_id = auth.uid() OR (EXISTS (SELECT 1 FROM admin_profiles WHERE id = auth.uid())));
...
CREATE POLICY "Usuários podem ver anexos de solicitações acessíveis" ON public.anexos
  FOR SELECT USING (EXISTS (
    ...
    AND (s.solicitante_id = auth.uid() OR s.aprovador_atual_id = auth.uid() OR (EXISTS (SELECT 1 FROM admin_profiles WHERE id = auth.uid())))
  ));
...
CREATE POLICY "Usuários podem criar registros de histórico" ON public.historico_aprovacao
  FOR INSERT WITH CHECK (usuario_id = auth.uid() OR (EXISTS (SELECT 1 FROM admin_profiles WHERE id = auth.uid())));
...
CREATE POLICY "Admins podem gerenciar fluxo de aprovadores" ON public.fluxo_aprovadores
  FOR ALL USING ((EXISTS (SELECT 1 FROM admin_profiles WHERE id = auth.uid()))) WITH CHECK ((EXISTS (SELECT 1 FROM admin_profiles WHERE id = auth.uid())));

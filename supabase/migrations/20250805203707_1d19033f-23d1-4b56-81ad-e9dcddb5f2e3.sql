...
-- 5. Adicionar unique constraint para evitar duplicatas (removendo o primeiro se existir)
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'unique_fpa_client_profile' 
    AND table_name = 'fpa_clients'
  ) THEN
    ALTER TABLE public.fpa_clients 
    ADD CONSTRAINT unique_fpa_client_profile UNIQUE (client_profile_id);
  END IF;
END $$;